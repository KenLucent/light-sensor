...
class ActionPanel(wx.Panel):
    ...
    def InitUI(self):
        ...
        self.AddMode(u'Показания датчика', SensorPanel(self, self.device))
...
class SensorPanel(wx.Panel):
    def __init__(self, parent, device):
        super(SensorPanel, self).__init__(parent)
        self.device = device
        self._stopped = True
        self.min_level = 0
        self.max_level = 1
    ...
    def InitUI(self):
        ...
        sizer.Add(wx.StaticText(self, label=u"Текущее значение"), 0, wx.EXPAND | wx.ALL, 10)
        self.sensor_label = wx.StaticText(self)
        self.sensor_label.SetLabel(u"0")
        sizer.Add(self.sensor_label, 0, wx.EXPAND | wx.ALL, 10)

        self.max_level_label = wx.StaticText(self)
        self.max_level_label.SetLabel(u"1")
        self.min_level_label = wx.StaticText(self)
        self.min_level_label.SetLabel(u"0")
        self.set_max_button = wx.Button(self, label=u"Установить максимум")
        self.set_min_button = wx.Button(self, label=u"Установить минимум")
        self.turn_on_button = wx.Button(self, label=u"Включить")
        self.turn_off_button = wx.Button(self, label=u"Отключить")

        self.turn_on_button.Bind(wx.EVT_BUTTON, self.OnActivateButton)
        self.turn_off_button.Bind(wx.EVT_BUTTON, self.OnDeactivateButton)
        self.set_max_button.Bind(wx.EVT_BUTTON, self.OnSetMaxButton)
        self.set_min_button.Bind(wx.EVT_BUTTON, self.OnSetMinButton)

        sizer.Add(self.set_max_button, 0, wx.EXPAND | wx.ALL, 10)
        sizer.Add(self.max_level_label, 0, wx.EXPAND | wx.ALL, 10)
        sizer.Add(self.set_min_button, 0, wx.EXPAND | wx.ALL, 10)
        sizer.Add(self.min_level_label, 0, wx.EXPAND | wx.ALL, 10)
        sizer.Add(self.turn_on_button, 0, wx.EXPAND | wx.ALL, 10)
        sizer.Add(self.turn_off_button, 0, wx.EXPAND | wx.ALL, 10)
        ...
        #self.sensor_label = wx.StaticText(self)
        #self.sensor_label.SetLabel(u"Поазания датчика")
        
        #box = wx.BoxSizer(wx.HORIZONTAL)
        #box.Add(self.sensor_label, 0, wx.EXPAND | wx.ALL, 10)
        
        #self.SetSizer(box)
    def OnSetMaxButton(self, event):
        self.max_level = self.device.get_analog()
        self.max_level_label.SetLabel(u"%.3f" % self.max_level)

    def OnSetMinButton(self, event):
        self.min_level = self.device.get_analog()
        self.min_level_label.SetLabel(u"%.3f" % self.min_level)

    def OnActivateButton(self, event):
        self.ActivateMode()

    def OnDeactivateButton(self, event):
        self.DeactivateMode()
